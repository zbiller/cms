<table
    ...
    data-order-url="{{ route('admin.attribute_sets.order') }}"
    data-order-model="{{ \App\Models\Shop\Attribute\Set::class }}"
        ...
        @if($items->count() > 0)
            @foreach($items as $index => $item)
                <tr id="{{ $item->id }}" class="{!! $index % 2 == 0 ? 'even' : 'odd' !!}">
                    ...
                    <td>{{ $item->slug ?: 'N/A' }}</td>
                    <td>
                        {!! button()->editRecord(route('admin.attribute_sets.edit', $item->id)) !!}
                        {!! button()->deleteRecord(route('admin.attribute_sets.destroy', $item->id)) !!}
                    </td>
                </tr>
            @endforeach
        @else
