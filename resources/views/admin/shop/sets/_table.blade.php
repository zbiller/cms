<table
    ...
    data-order-url="{{ route('admin.sets.order') }}"
    data-order-model="{{ \App\Models\Shop\Set::class }}"
        ...
        @if($items->count() > 0)
            @foreach($items as $index => $item)
                <tr id="{{ $item->id }}" class="{!! $index % 2 == 0 ? 'even' : 'odd' !!}">
                    ...
                    <td>{{ $item->slug ?: 'N/A' }}</td>
                    <td>
                        {!! button()->editRecord(route('admin.sets.edit', $item->id)) !!}
                        {!! button()->deleteRecord(route('admin.sets.destroy', $item->id)) !!}
                    </td>
                </tr>
            @endforeach
        @else
